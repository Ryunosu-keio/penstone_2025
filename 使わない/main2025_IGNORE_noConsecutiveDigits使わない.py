...
STATUS_LABEL_MAP = {
    1: "Match",
    2: "Mismatch",
    4: "Ignore"  # Frontがアルファベット（押さないのが正解）
}
...
def generate_trial_data(num_trials, available_images):
    """
    48試行を生成する（一般化して num_trials にも対応）。

    仕様（最重要）:
    - ターゲット(Front=数字) : 全体の 1/3
      - Match (status=1) と Mismatch (status=2) は 50:50
    - Ignore(Front=アルファベット) : 全体の 2/3 (status=4)
    - Back画像は全試行で必ず表示（Ignoreでも表示）
    - 「Frontが数字の試行（ターゲット）」が連続しないように並べる
      → ターゲット同士が隣り合わない（必ず間にIgnoreが入る）

    返す列:
      - status     : int (1,2,4)
      - front_char : str (digit or letter)
      - back_char  : str (digit)  ※Back画像の意味（0-9）
      - image_path : str (元画像パス or "dummy.jpg")
    """
    if num_trials <= 0:
    ...
    # --- 1/3 ターゲット、2/3 Ignore を“厳密に”作る（48なら 16/32）---
    if num_trials % 3 == 0:
        n_target = num_trials // 3
    else:
        n_target = int(round(num_trials / 3))
        n_target = max(0, min(num_trials, n_target))

    n_ignore = num_trials - n_target

    # ターゲット内訳（半々）
    n_match = n_target // 2
    n_mismatch = n_target - n_match

    # --- ターゲット試行（数字）を作る ---
    target_trials = []
    status_list = ([1] * n_match) + ([2] * n_mismatch)
    random.shuffle(status_list)

    for status in status_list:
        front_digit = random.choice(digits)
        if status == 1:
            back_digit = front_digit
        else:
            back_digit = random.choice([d for d in digits if d != front_digit])

        # Back画像は back_digit から選ぶ（意味の数字に対応）
        if available_images.get(back_digit):
            img_path = random.choice(available_images[back_digit])
        else:
            img_path = "dummy.jpg"

        target_trials.append({
            "status": status,
            "front_char": front_digit,
            "back_char": back_digit,
            "image_path": img_path
        })

    # --- Ignore試行（アルファベット）を作る（連続同一アルファベットを避ける） ---
    ignore_trials = []
    prev_letter = None
    for _ in range(n_ignore):
        cand = [c for c in letters if c != prev_letter]
        front_letter = random.choice(cand)
        prev_letter = front_letter

        back_digit = random.choice(digits)
        if available_images.get(back_digit):
            img_path = random.choice(available_images[back_digit])
        else:
            img_path = "dummy.jpg"

        ignore_trials.append({
            "status": 4,
            "front_char": front_letter,
            "back_char": back_digit,
            "image_path": img_path
        })

    # --- 並びを作る（ターゲットが連続しないように挿入） ---
    # Ignoreが n_ignore 個なら「隙間」は n_ignore+1 個。そこにターゲットを1個ずつ置けば連続しない。
    gaps = [[] for _ in range(n_ignore + 1)]
    gap_indices = random.sample(range(n_ignore + 1), k=len(target_trials))
    for t, gi in zip(target_trials, gap_indices):
        gaps[gi].append(t)

    trials = []
    for i in range(n_ignore):
        trials.extend(gaps[i])
        trials.append(ignore_trials[i])
    trials.extend(gaps[n_ignore])

    # --- 整合性チェック ---
    if len(trials) != num_trials:
        raise RuntimeError("Internal error: trial length mismatch")

    # ターゲットが連続しないこと（Frontが数字の試行が隣り合わない）
    for i in range(len(trials) - 1):
        if str(trials[i]["front_char"]).isdigit() and str(trials[i+1]["front_char"]).isdigit():
            raise RuntimeError("Internal error: consecutive digit trials detected (should not happen)")
        ...
        for set_num in tqdm(range(20), desc=f"Processing {cond_label}"):
            ...
            for i, row in tqdm(df.iterrows(), total=len(df), desc=f"Set {set_num}", leave=False):
                ...
                data_row = {
                    ...
                    "status": status_str,
                    "front_char": row.get("front_char",""),
                    "back_char": row.get("back_char",""),
            ...
            cols_back = [
                ...
                "status", "front_char", "back_char", "image_name"
            ]
