# # -*- coding: utf-8 -*-
# """
# add_emr_columns_to_experiment_log_v2.py
# --------------------------------------
...
# EXPERIMENT_LOG_CSV = "../../log/Dark/S01/S01_0.csv"
# EMR_SEGMENT_CSV    = "../../data/devided_emr/1/0.csv"
# OUT_LOG_CSV        = "../../data/integrated_2025/Dark/S01/S01_0_with_emr.csv"
# OUT_PNG            = "../../data/graphs/integrated_2025/Dark/S01/S01_0_alignment_check.png"
...
# # 平滑化（rolling）窓（フレーム数）
# # 60Hzなら 5~11、120Hzなら 9~21 あたりが無難
# SMOOTH_WIN = 7

# # diopter の有効レンジ（元コードの思想：範囲外は0）
# DIOPTER_MIN = 1.5
# DIOPTER_MAX = 10.0

# # ★「元コード踏襲」：刺激提示フレームから、上昇が止まる地点（局所最大）を取る探索窓
# # 元コードの t += 240 は（120Hz想定で）2秒ぶん。
# PEAK_WINDOW_SEC = 2.0
# # ============================================================
...
# def _smooth_series(s: pd.Series, win: int) -> pd.Series:
#     # rolling median → rolling mean の2段（外れ値に強くしてから滑らかに）
#     s = pd.to_numeric(s, errors="coerce")
#     if win <= 1:
#         return s
#     med = s.rolling(win, center=True, min_periods=max(1, win // 2)).median()
#     out = med.rolling(win, center=True, min_periods=max(1, win // 2)).mean()
#     return out
...
#     df["emr_both_Z_mm_raw"] = df["両眼.注視Z座標[mm]"]
...
#     # ---- 前処理（外れ値→NaN → 平滑化） ----
#     left  = _clip_to_nan(df["emr_left_pupil_mm_raw"],  PUPIL_MIN_MM, PUPIL_MAX_MM)
#     right = _clip_to_nan(df["emr_right_pupil_mm_raw"], PUPIL_MIN_MM, PUPIL_MAX_MM)
#     zc    = _clip_to_nan(df["emr_both_Z_mm_raw"],       Z_MIN_MM, Z_MAX_MM)

#     left_s  = _smooth_series(left,  SMOOTH_WIN)
#     right_s = _smooth_series(right, SMOOTH_WIN)
#     z_s     = _smooth_series(zc,    SMOOTH_WIN)
...
#     # diopter（前処理後Zから）
#     z2 = pd.to_numeric(df["emr_both_Z_mm"], errors="coerce")
#     diop = np.where((z2 > 0) & (~np.isnan(z2)), 1000.0 / z2, np.nan)
#     diop = pd.Series(diop)

#     # ---- 元コード思想：範囲外は 0 にする ----
#     diop0 = diop.copy()
#     diop0 = diop0.where((diop0 >= DIOPTER_MIN) & (diop0 <= DIOPTER_MAX), 0.0)
#     # diopterも少し平滑化（0があるので median の方が効く）
#     diop0_s = _smooth_series(diop0.replace(0.0, np.nan), SMOOTH_WIN).fillna(0.0)
...
#     # start が 0 の場合もあるので、まず max_search の範囲で動かす
#     while j < max_search and (start_idx + j) < n:
#         v = float(diop0[start_idx + j])
#         last_idx = start_idx + j

#         # 元コード：while 条件は「max < v」 OR 「v == 0」
#         # ここでは break 条件として「v が 0 ではなく、かつ mx >= v」になったら上昇終了とみなす
#         if (v != 0.0) and (mx >= v):
#             break
...
#     """
#     merged（ログにEMRが付いたもの）に、刺激フレームごとの diopter_peak を追加。
#     """
#     # 刺激フレーム（ログ側）
...
#     # 各刺激フレームに対して、最も近いEMRフレーム idx を起点に peak を取る
...
#         # searchsorted は「右側」になりやすいので、左右比較で最近傍に寄せる
...
#     # すでに emr_* 列があるログ（OUT_LOG_CSVなど）を読んでも重複しないように削る
...
#     - save_dir を指定すると pageごとにPNG保存
#     """

#     df = pd.read_csv(log_with_emr_csv, encoding="utf-8-sig")

#     # frame_col はあなたの関数を使う（EMR_FPSに合わせて選ぶ）
...
#     # 刺激フレーム（縦線）：重複しないように unique & sort
...
#         eidx = min((page + 1) * trials_per_page, n_trials)  # 終端（非包含）
...
#         # 1) left pupil
#         axes[0].plot(x_emr, emr_df["emr_left_pupil_mm"].values, linewidth=1.0)
#         _scatter(axes[0], df[frame_col].values, df.get("emr_left_pupil_mm", np.nan), s=12)
#         axes[0].set_ylabel("Left pupil [mm]")
#         axes[0].set_title(f"EMR alignment (page {page+1}/{n_pages})  xlim={x0:.0f}-{x1:.0f}")

#         # 2) right pupil
#         axes[1].plot(x_emr, emr_df["emr_right_pupil_mm"].values, linewidth=1.0)
#         _scatter(axes[1], df[frame_col].values, df.get("emr_right_pupil_mm", np.nan), s=12)
#         axes[1].set_ylabel("Right pupil [mm]")

#         # 3) both pupil
#         axes[2].plot(x_emr, emr_df["emr_both_pupil_mm"].values, linewidth=1.0)
#         _scatter(axes[2], df[frame_col].values, df.get("emr_both_pupil_mm", np.nan), s=12)
#         axes[2].set_ylabel("Both pupil [mm]")

#         # 4) diopter
#         axes[3].plot(x_emr, emr_df["emr_diopter_clipped0"].values, linewidth=1.0)
...
#         if save_dir:
#             out_path = os.path.join(save_dir, f"alignment_page_{page+1:02d}.png")
...
#         pad_right_frames=180,        # 右余白（2.5秒ぶん=150が見えるように少し多め）
#         save_dir="../../data/graphs/pages",  # 保存したいなら
#         show=True,
#     )
