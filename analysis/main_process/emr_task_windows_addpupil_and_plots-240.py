...
def load_emr_with_diopter(
    ...
    # ★ Z座標の前処理
    z_s = preprocess(df["両眼.注視Z座標[mm]"], Z_MIN_MM, Z_MAX_MM)
...
def main():
    ...
    DEFAULT_OUT_ROOT = "../../data/integrated_2025_metrics"
    ...
    # Bright:  S01～S04, セグメント0～9 (★被験者ごとにtask_sec変更)
    # for s in range(1, 20):
    ...
    for env, subject_num, task_num in configs:
        # ★一桁の場合は0埋め（S01, S02, ... ）
...
def process_single_file(log_csv, emr_csv, args):
    ...
    out_csv = os.path.join(args.out_root, cond, subj, f"{base}_with_emr_metrics.csv")
