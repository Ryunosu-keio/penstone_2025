...
def extract_red_imagename(room) :
    if room == "bright":
        ...
        df_path_list = glob.glob("../histogram/redpic_dark/*.csv")
    df_list = []
    red_grid_list = []
...
def hist_list_by_df(df):
    ...
    return img_list, hist_list, image_path_list, diopter_list

#matplotlibで10n枚ずつ表示する
def show_hists(red_grid_list,room):
    # ここでは例として、画像の数を20と仮定
    ...
    if room == "bright":
        ...
        plt.savefig("../histogram/redpic_dark/hist_red_dark.pdf")
    plt.show()
