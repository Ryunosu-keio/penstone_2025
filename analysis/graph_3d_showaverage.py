        ...
        for j in range(len(y_grid)-1):
            for k in range(len(z_grid)-1):
                ...
                if not grid_data.empty:
                    ...
                    # グリッドを描画
                    ax.scatter(center_x, center_y, center_z, color=color, s=50)
...
# グリッドの描画（'brightness', 'contrast', 'gamma' を例として使用）
# plot_3d_with_grids(df, 'gamma', 'sharpness', 'equalization')
# plot_3d_with_grids(df, 'gamma', 'contrast', 'sharpness')
# plot_3d_with_grids(df, 'contrast', 'sharpness', 'equalization')
# plot_3d_with_grids(df, "contrast", "sharpness", "equalization")
plot_3d_with_grids(df, "gamma", "sharpness", "equalization")
