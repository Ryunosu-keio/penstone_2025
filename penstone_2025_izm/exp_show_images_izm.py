...
# ------------------------------------------------------------
# フィラー（アルファベット）試行で固定画像を使うか
#   True  → cond別の固定画像を表示（従来動作）
#   False → 通常画像（Excelのimage_name）をそのまま表示
USE_FILLER_FIXED_IMAGE =False

# アルファベット時に出す固定画像（cond別） ※ USE_FILLER_FIXED_IMAGE=True 時のみ使用
...
TARGET_KEY_MAP = {"Match": "t", "Mismatch": "f"}
...
def main():
    ...
    # Excel
    EXCEL_ROOT = "./imageCreationExcel_izm"
    excel_path = os.path.join(EXCEL_ROOT, cond_label, participant_id, f"{set_num}.xlsx")

    # Images
    IMAGE_ROOT = pick_existing_dir(
        r"F:\experiment_images",
        r"D:\experiment_images",
        r"G:\experiment_images",
    )
    if IMAGE_ROOT == "":
        print(r"\nエラー: experiment_images が F:\ / D:\ / G:\ にありません")
        return
    ...
    keyboard.on_press_key("f", on_key_event, suppress=True)
    ...
    # アルファベット固定画像（cond別）を読み込む（USE_FILLER_FIXED_IMAGE=True の場合のみ）
    fixed_path = ALPHABET_FIXED_IMAGE_PATHS.get(cond_label, "")
    if USE_FILLER_FIXED_IMAGE:
        ...
        print(f"[INFO] USE_FILLER_FIXED_IMAGE=False → フィラーも通常画像を使用")
    ...
    try:
        for i in range(len(df)):
            ...
            try:
                if is_alphabet_trial and USE_FILLER_FIXED_IMAGE:
            ...
            if status == "Ignore":
                ...
                correct_key = TARGET_KEY_MAP.get(status, "f")
                if user_key is None:
