...
# ============================================================
# 入力データ選択（merged_for_grid から対話式）
# ============================================================
grid_data_root = "../../data/merged_for_grid"

# パラメータフォルダ一覧
param_dirs = []
if os.path.isdir(grid_data_root):
    for d in sorted(os.listdir(grid_data_root)):
        full = os.path.join(grid_data_root, d)
        if os.path.isdir(full):
            param_dirs.append(d)

if not param_dirs:
    raise FileNotFoundError(f"merged_for_grid にフォルダがありません: {grid_data_root}")

print("\n=== 利用可能な計算パラメータ ===")
for i, name in enumerate(param_dirs, 1):
    print(f"  {i}. {name}")

p_idx = int(input(f"\n使用するパラメータを選択 (1-{len(param_dirs)}): ").strip()) - 1
chosen_param = param_dirs[p_idx]

# Bright / Dark 選択
cond_dirs = []
param_path = os.path.join(grid_data_root, chosen_param)
for d in sorted(os.listdir(param_path)):
    if os.path.isdir(os.path.join(param_path, d)):
        cond_dirs.append(d)

print(f"\n=== 条件選択 ({chosen_param}) ===")
for i, name in enumerate(cond_dirs, 1):
    print(f"  {i}. {name}")

c_idx = int(input(f"\n条件を選択 (1-{len(cond_dirs)}): ").strip()) - 1
chosen_cond = cond_dirs[c_idx]

# Excelファイルを探す
cond_path = os.path.join(param_path, chosen_cond)
xlsx_files = sorted([f for f in os.listdir(cond_path) if f.endswith(".xlsx")])
if not xlsx_files:
    raise FileNotFoundError(f"Excelファイルが見つかりません: {cond_path}")

path = os.path.join(cond_path, xlsx_files[0])
print(f"\n[INFO] 読み込み: {path}")
