...
def devide_emrLog(num):
    ...
    for emrfile in emrfiles:
        ...
        for i in range(len(emr_df) - 1):
            if not pd.isna(emr_df.iloc[i][EMR_COL_CUE]) and pd.isna(emr_df.iloc[i+1][EMR_COL_CUE]):
                start = i + 1
                in_interval = True
            elif in_interval and not pd.isna(emr_df.iloc[i][EMR_COL_CUE]):
                end = i
        ...
        for i in range(len(cue_intervals_emrfile)-1, -1, -1):
            if cue_intervals_emrfile[i][1] - cue_intervals_emrfile[i][0] < 14400: #120Hzで83秒未満のデータは削除　2.5sなら14400,3.75sなら21600　5sなら28800
